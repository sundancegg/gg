...
const requestLoginTokenButton = document.getElementById('requestPermissions')
const getSecretPage = document.getElementById('getPermissions')
...
const getTempleStats = document.getElementById('getTempleStats')
const templeAddressInput = document.getElementById('templeAddressInput')
const encryptButton = document.getElementById('encryptButton')
const decryptButton = document.getElementById('decryptButton')
const templeGoddessesDisplay = document.getElementById('templeGoddessesDisplay')
const templePatronsDisplay = document.getElementById('templePatronsDisplay')
const templeNameDisplay = document.getElementById('templeNameDisplay')
const templeBaseRateDisplay = document.getElementById('templeBaseRateDisplay')
...
const initialize = async () => {
  ...
  const clearTextDisplays = () => {
    templeGoddessesDisplay.innerText = ''
    templeAddressInput.value = ''
    templePatronsDisplay.innerText = ''
    templeNameDisplay.innerText = ''
  }
  ...
  const initializeAccountButtons = () => {
     ...
     * Permissions
     */
        requestLoginTokenButton.onclick = async () => {
            try {
                console.log("requestLoginTokenButton", `/nonce/${accounts[0]}`); 
                let token_phrase
                const response = await (await fetch(`/auth/${accounts[0]}`)).json()
                                  
                token_phrase = response["token_phrase"]
                console.log('token_phrase', token_phrase);

                const from = accounts[0];
                const msg = `0x${Buffer.from(token_phrase, 'utf8').toString('hex')}`;
                const sign = await ethereum.request({
                  method: 'personal_sign',
                  params: [msg, from, 'Example xx password'],
                })
               
                const auth_response = await ( await fetch(`/auth/${accounts[0]}`, {
                  method: 'POST',
                  headers: {
                ...
                permissionsResult.innerHTML = auth_response
                console.log("auth return: ", auth_response)                  
    ...
    getSecretPage.onclick = async () => {
      try {
        const response = await( await fetch("/secret", {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': localStorage.getItem('access_token_cookie')
            }
          })).text()
          permissionsResult.innerHTML = response
    ...
    getAccountsButton.onclick = async () => {
      try {
        eth_accounts = await ethereum.request({
          method: 'eth_accounts',
        })
        getAccountsResults.innerHTML = eth_accounts[0] || 'Not able to get accounts'
      } catch (err) {
        console.error(err)
    ...
    getTempleStats.onclick = async () => {
      try {
        const response = await ( await fetch(`/temple/${templeAddressInput.value}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            },
          body: JSON.stringify({
            "signature": "xyz...whatever" //sign,
          })
        })).json()
        templeGoddessesDisplay.innerText = response.goddesses
        templePatronsDisplay.innerText = response.patrons
        templeNameDisplay.innerText = response.name
        templeBaseRateDisplay.innerText = response.base_rate


      } catch (error) {
        templeGoddessesDisplay.innerText = `Error: ${error.message}`
        
      }
    }

    templeAddressInput.onkeyup = () => {
      if (
        !getTempleStats.disabled &&
        templeAddressInput.value.length > 0
      ) {
    ...
    encryptButton.onclick = () => {
      try {
        templePatronsDisplay.innerText = web3.toHex(JSON.stringify(
          encrypt(
            templeGoddessesDisplay.innerText,
            { 'data': templeAddressInput.value },
            'x25519-xsalsa20-poly1305',
          ),
        ))
        decryptButton.disabled = false
      } catch (error) {
        templePatronsDisplay.innerText = `Error: ${error.message}`
    ...
    decryptButton.onclick = async () => {
      try {
        templeNameDisplay.innerText = await ethereum.request({
          method: 'eth_decrypt',
          params: [templePatronsDisplay.innerText, ethereum.selectedAddress],
        })
      } catch (error) {
        templeNameDisplay.innerText = `Error: ${error.message}`
      }
    }
  }
...
},{"@metamask/onboarding":2,"buffer":5}],2:[function(require,module,exports){
'use strict';
